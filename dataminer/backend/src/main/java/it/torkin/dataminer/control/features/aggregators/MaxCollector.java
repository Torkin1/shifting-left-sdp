...
public class MaxCollector implements Collector<Feature<? extends Number>, Holder<Double>, DoubleFeature> {

    @Override
    public BiConsumer<Holder<Double>, Feature<? extends Number>> accumulator() {
        return (holder, feature) -> holder.setValue(feature.getValue().doubleValue() > holder.getValue() ? feature.getValue().doubleValue() : holder.getValue());
    }
    ...
    public BinaryOperator<Holder<Double>> combiner() {
        return (holder1, holder2) -> new Holder<>(holder2.getValue() > holder1.getValue() ? holder2.getValue() : holder1.getValue());
    }
    ...
    public Supplier<Holder<Double>> supplier() {
        return () -> new Holder<Double>(Double.MIN_VALUE);
    }
