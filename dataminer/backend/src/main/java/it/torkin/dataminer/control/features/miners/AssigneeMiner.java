...
public class AssigneeMiner extends FeatureMiner{
    ...
    public void mine(FeatureMinerBean bean) {
        ...
        Double anfic = Double.NaN;
        Double familiarity = Double.NaN;
        ...
        // mine assignee features only if measurement date is not before assignement
        if (bean.getMeasurementDate().getClass() != OneSecondBeforeFirstAssignmentDate.class){
            // gets the assignee key of the issue, if available
            String assigneeKey = issueController.getAssigneeKey(new IssueBean(bean.getIssue(), bean.getMeasurement().getMeasurementDate()));
            if (!StringTools.isBlank(assigneeKey)){
